...
# import classify
import xlsxwriter
from . import utils, classify


def main(argv=None):
    parser = argparse.ArgumentParser(
        prog="tribus", description="Execute tribus options on single cell data tables"
    )
    subparsers = parser.add_subparsers(
        title="tribus subcommands",
        help="Additional sub-command help with -h",
        dest="command",
    )
    ...
    parser_classify = subparsers.add_parser(
        "classify", help="Assign cell type labels to cells"
    )
    parser_preview = subparsers.add_parser(
        "preview", help="Preview data with no labels"
    )
    parser_report = subparsers.add_parser(
        "report", help="Report quality metrics for cell type labels"
    )
    ...
    parser_report.add_argument(
        "--resultFolder", metavar="DIR", help="Generate quality report after labeling"
    )
...
def run_tribus_from_file(
    ...
    if valid and valid_depth:
        ...
        output_folder = os.path.join(
            output, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M")
        )
        ...
        writer = pd.ExcelWriter(
            f"{output_folder}/expected_phenotypes.xlsx", engine="xlsxwriter"
        )
        for key in logic:
...
def run_tribus(
    ...
    if valid_input and valid_logic and valid_depth:
        tree = utils.build_tree(logic, depth)
        result_table, prob_table = classify.run(
            input_df,
            logic,
            depth,
            pd.DataFrame(),
