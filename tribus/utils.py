...
def validate_input_data(file, logic):
    ...
    valid = True
    markers_in_logic = np.unique(
        [marker for key in logic for marker in logic[key].index]
    )
...
def traverse(
    graph, node, old_logic, logic, previous_labels, reusable_labels, old_levels
):
    """
    ...
    if (
        node in old_levels
        and old_logic[node].equals(logic[node])
        and node in previous_labels.columns
    ):
        reusable_labels[node] = previous_labels[node]
        out_edges = graph.out_edges(node)
        for i, j in out_edges:
            traverse(
                graph, j, old_logic, logic, previous_labels, reusable_labels, old_levels
            )
...
def re_entry(output, logic, depth, sample_name, tree, output_folder):
    ...
    for path in os.listdir(output):
        if (
            not os.path.isfile(os.path.join(output, path))
            and path != output_folder.split("/")[-1]
        ):
            folders.append(path)
...
def run_classify(
    ...
    for file in input_files:
        ...
        start = time.time()
        if save_figures:
            result_labels, prob_table = classify.run(
                input_files[file],
                logic,
                depth,
                previous_labels,
