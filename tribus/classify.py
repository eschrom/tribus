...
def hyperparameter_tuning(sample_data, max_evals):
    ...
    def som_fn(space):
        ...
        val = MiniSom(
            x=x, y=x, input_len=input_len, sigma=sig, learning_rate=learning_rate
    ...
    print(
        "Current grid size x is {}, grid size y is {}, sigma is {}, learning rate is {}.".format(
            x, y, sigma, learning_rate
        )
    )
...
def score_nodes(data_to_score, logic, level):
    ...
    for idx, cell_type in enumerate(logic[level].columns.values):
        list_negative = list(
            level_logic_df.loc[level_logic_df[cell_type] == -1].index
        )  # get markers with negative scores
        list_positive = list(
            level_logic_df.loc[level_logic_df[cell_type] == 1].index
        )  # get markers with positive scores

        if list_positive == 0:
            continue

        gating_positive = data_to_score[
            list_positive
        ].to_numpy()  # rows: clusters, columns: positive markers
        marker_scores_positive = np.apply_along_axis(
            score_marker_pos, 0, gating_positive
        )

        if len(list_negative) != 0:
            gating_negative = data_to_score[list_negative].to_numpy()
            marker_scores_negative = np.apply_along_axis(
                score_marker_neg, 0, gating_negative
            )

            marker_scores = np.column_stack(
                (marker_scores_positive, marker_scores_negative)
            )
        else:
            marker_scores = marker_scores_positive

        normalized_marker_scores = np.apply_along_axis(
            normalize_scores, 0, marker_scores
        )
        scores_matrix[:, idx] = np.mean(
            normalized_marker_scores, 1
        )  # put the mean of the marker values of a celltype into a matrix (indexed by the celltypes)
    scores_pd = pd.DataFrame(
        scores_matrix, columns=logic[level].columns.values, index=data_to_score.index
    )
    return scores_pd
...
def clustering(
    ...
    if len(sample_data) < clustering_threshold:
        ...
        scores_labels["cell_label"] = scores_pd.apply(
            lambda x: get_cell_type(x, level, undefined_threshold, other_threshold),
            axis=1,
        )
        ...
        data_to_score, labeled = cluster_cells(
            sample_data, logic, level, sigma, learning_rate, random_state
        )
        scores_pd = score_nodes(data_to_score, logic, level)
        ...
        scores_labels["probability"] = scores_pd.apply(get_probabilities, axis=1)
        labels_list = scores_labels.loc[
            labeled["label"]
        ].cell_label  # according to the cluster labels, assign the most probable cell-type to each cell
    ...
    if current_depth < max_depth:
        ...
        if node in previous_labels.columns:
            ...
            filtered_markers = list(
                logic[node].loc[(logic[node] != 0).any(axis=1)].index
            )
            ...
            if tuning != 0:
                ...
                sigma, learning_rate = hyperparameter_tuning(
                    sample_data, max_evals=max_evals
                )
            else:
                sigma = sigma
                learning_rate = learning_rate

            result, prob = clustering(
                data_subset,
                logic,
                node,
                sigma,
        ...
        if output is not None:
            ...
            visualization.correlation_matrix(
                data_subset,
                markers=list(logic[node].index),
                level=node,
                save=True,
                fname=f"{output}/{sample_name}_correlation_{node}",
            )

            visualization.heatmap_for_median_expression(
                data_subset,
                result_table,
                logic,
                level=node,
            ...
            visualization.umap_vis(
                data_subset,
                result_table,
                markers=markers,
                save=True,
                fname=f"{output}/{sample_name}_umap_{node}",
                level=node,
            )

            visualization.marker_expression_by_cell_type(
                data_subset,
                result_table,
                markers=list(logic[node].index),
                level=node,
                save=True,
                fname=f"{output}/{sample_name}_marker_expression_{node}.png",
            )

            visualization.cell_type_distribution(
                result_table,
                level=node,
                save=True,
                fname=f"{output}/{sample_name}_barplot_{node}.png",
            )
            print("Figures done")

        out_edges = tree.out_edges(node)
        for i, j in out_edges:
            result_table, prob_table = traverse(
                tree,
                sample_data,
                logic,
                max_depth,
                ...
                undefined_threshold=undefined_threshold,
...
def run(
    ...
    if output is not None:
        ...
        sample_data[markers]
        visualization.marker_expression(
            sample_data[markers],
            save=True,
            fname=f"{output}/{sample_name}_markerexpression.png",
        )
